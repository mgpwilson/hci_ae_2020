...
export class Pandemic {
  ...
  constructor(
    casesOnDay0,
    infectedAvgExposures,
    probInfectFromExpose,
    popSize,
    hospitalCapacity
  ) {
  ...
  getRecoveredByDay(dayNum) {
    ...
    let willDie = this.retrieveCachedCalculation(
  ...
  getSusceptibleByDay(dayNum) {
    let susceptible =
      this.popSize -
      this.retrieveCachedCalculation(dayNum, "cases") -
      this.retrieveCachedCalculation(dayNum, "total_recovered") -
      this.retrieveCachedCalculation(dayNum, "total_deaths");
  ...
  getAdjustedInfectedAvgExposures() {
    return (
      this.infectedAvgExposures *
      this.factors.close_education *
      this.factors.public_transport_reduced *
      this.factors.avoid_groups
    );
  }

  getAdjustedProbInfectFromExpose() {
    return (
      this.probInfectFromExpose *
      this.factors.handWashing *
      this.factors.masks *
      this.factors.outdoor_socialising *
      this.factors.socialDistancing
    );
  }

  getRValue() {
    return (
      this.getAdjustedInfectedAvgExposures() *
      this.getAdjustedProbInfectFromExpose()
    );
  }
  ...
  getHospitalCapacityByDay(dayNum) {
    if (dayNum > this.simulationLength) {
      ...
      for (let i = dayNum - this.avgLengthOfInfection; i < dayNum; i++) {
        ...
        if (recentInfections > this.popSize)
          recentInfections = this.hospitalCapacity;
      }
      ...
      let recent = this.memoizedData.cases.slice(
        dayNum - this.avgLengthOfInfection,
        dayNum
      );
      let recentInfections = 0;
      ...
      for (let i = 0; i < recent.length; i++) {
        ...
        if (recentInfections > this.hospitalCapacity)
          recentInfections = this.hospitalCapacity;
      }
  ...
  getDeathProportionalToPopulation(dayNum) {
    return (
      this.retrieveCachedCalculation(dayNum, "total_deaths") / this.popSize
    );
  }
  ...
  seriesCasesByDay() {
    ...
    for (let i = 0; i < 120; i++) {
      ...
      s.push({
        x: i,
        y: Math.round(this.retrieveCachedCalculation(i, "cases")),
      });
    }
    return s;
  }
  ...
  seriesRecoveredByDay() {
    ...
    for (let i = 0; i < 120; i++) {
      ...
      s.push({
        x: i,
        y: Math.round(this.retrieveCachedCalculation(i, "total_recovered")),
      });
    }
    return s;
  }
  ...
  seriesSusceptibleByDay() {
    ...
    for (let i = 0; i < 120; i++) {
      ...
      s.push({
        x: i,
        y: Math.round(this.retrieveCachedCalculation(i, "susceptible")),
      });
    }
    return s;
  }
  ...
  seriesDeathsByDay() {
    ...
    for (let i = 0; i < 120; i++) {
      ...
      s.push({
        x: i,
        y: Math.round(this.retrieveCachedCalculation(i, "total_deaths")),
      });
    }
    return s;
  }
...
export const HospitalCapacityContextualisation = (props) => {
  ...
  if (daysUntil > 0) {
    ...
    message =
      "Hospitals beds would not run out. No severely ill or dying person would be turned away.";
  }
